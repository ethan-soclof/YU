...
public class CommandSet<Target> extends AbstractSet<GenericCommand<Target>> implements Undoable {
    ...
    public boolean containsTarget(Target c){
        for (GenericCommand command: this.genericCommands){
            if (command.getTarget().equals(c)){
                return true;
    ...
    public Set<GenericCommand<Target>> undoAll(){
        ...
        List<GenericCommand<Target>> list = new ArrayList<GenericCommand<Target>>(this.genericCommands);

        for (int i = 0; i < list.size(); i++){
            if (list.get(i).undo()){
                undone.add(list.get(i));
                this.genericCommands.remove(list.get(i));
            }
        }
